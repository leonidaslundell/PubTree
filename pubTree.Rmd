...
observeEvent(input$go|input$forward|input$back|input$delete|as.numeric(as.factor(input$summarize)),{#
  ...
  if(input$summarize == "Overview"){
    ga_collect_event(event_category = "View", event_action = "Viewed in Overview mode")
    ...
    ga_collect_event(event_category = "View", event_action = "Viewed in Standard mode")
    
    data$gg <- net(results = data$data[[data$i]]) |> 
      withProgress(message = "Clustering articles", 
                   value = .5)
